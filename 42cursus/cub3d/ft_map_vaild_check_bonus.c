...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_map_vaild_check_bonus.c                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: joopark <joopark@student.42seoul.kr>       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/09 22:28:46 by joopark           #+#    #+#             */
/*   Updated: 2021/01/12 19:32:59 by joopark          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				ft_dfs(t_map *map, int x, int y, int *is_false)
{
	...
	else if (map->map[y][x] >= '2' && map->map[y][x] <= '4')
	{
		if (0 == x || 0 == y || map->x == x || map->y == y)
			*is_false = *is_false + 1;
		map->map[y][x] = 'b' + (map->map[y][x] - '2');
	}
